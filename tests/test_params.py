...
class TestParams(unittest.TestCase):
    ...
    def test_suggest_int_fails_with_non_float_and_non_integer_values(self):
         with self.assertRaises(Exception):
            Params.suggest_int('hi', dict())
         with self.assertRaises(Exception):
            Params.suggest_int(1, 'hi')
         with self.assertRaises(Exception):
            Params.suggest_int('hi', 3)
         with self.assertRaises(Exception):
            Params.suggest_int(list(), 'hi')
         with self.assertRaises(Exception):
            Params.suggest_int(set(), 2)
         with self.assertRaises(Exception):
            Params.suggest_int(dict(), set())

    def test_suggest_float_fails_with_non_float_and_non_integer_values(self):
        with self.assertRaises(Exception):
            Params.suggest_float('hi', dict())
        with self.assertRaises(Exception):
            Params.suggest_float(1, 'hi')
        with self.assertRaises(Exception):
            Params.suggest_float('hi', 3)
        with self.assertRaises(Exception):
            Params.suggest_float(list(), 'hi')
        with self.assertRaises(Exception):
            Params.suggest_float(set(), 2)
        with self.assertRaises(Exception):
             Params.suggest_float(dict(), set())

    def test_suggest_categorical_fails_with_non_string_values(self):
        with self.assertRaises(Exception):
            Params.suggest_categorical([1, 3, 'hi'])
        with self.assertRaises(Exception):
            Params.suggest_categorical(['hello', 3, 'hi'])
        with self.assertRaises(Exception):
